<script lang="ts">
	...
	const company_name = $page.params.company_name;

	let ads: any[] = [];

	async function getCompanyAds() {
		const url = `http://127.0.0.1:8000/advertisement/company/${encodeURIComponent(company_name)}`;
